...
- `alignment/`
  - `<sample>.sorted.bam`: coordinate-sorted GRCh38 alignment of a FASTQ sample (bwa-mem2 for DNA, STAR for RNA), used as input for HLA\*LA and SpecHLA.
  - `<sample>.sorted.bam.bai`: BAM index.
- `reference/` (only with `--save_reference`)
  - `bwamem2/`: bwa-mem2 index built from the reference FASTA.
  - `star/`: STAR genome index (annotation-aware if `--gtf` was given).
  - `*.fai`: samtools FASTA index.

</details>

FASTQ samples are aligned to GRCh38 only when `--tools` includes `hlala` or `spechla`; samplesheet BAMs skip this step. The reference comes from `--genome hg38` or `--fasta`. samtools `stats`, `flagstat` and `idxstats` of the aligned BAMs are shown in the MultiQC report.

### HLA\*LA

[HLA\*LA](https://github.com/DiltheyLab/HLA-LA) (HLA typing using a graph-based reference) performs HLA typing from a genome-aligned BAM using a population reference graph of the MHC region. It supports typing of Class I and Class II HLA alleles.

> **Note:** HLA\*LA runs on samplesheet BAMs and on DNA FASTQ samples that the pipeline aligns to GRCh38 (see [Genome alignment](#genome-alignment)); RNA samples are skipped. The tool uses a large graph reference (~5 GB) that is either downloaded automatically or provided by the user.
